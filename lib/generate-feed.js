...
module.exports = function(parsedFeed, elements, dropElements, token, pureHtml) {
  return co(function*() {
    let feed = new RSS({
    ...
    for (let i of parsedFeed.articles) {
      let body = yield getFullText(i.link, elements, dropElements,
                                   token, pureHtml);
